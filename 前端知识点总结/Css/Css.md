...
（1）id选择器（#myid）
（2）类选择器（.myclassname）
（3）并集选择器以及标签选择器（div,h1,p），并集选择权就是选择多个
（4）后代选择器（h1 p）
（5）相邻后代选择器（子）选择器（ul>li）
（6）兄弟选择器（li~a）  li和a同属于一个父元素即可
（7）相邻兄弟选择器（li+a） li和a同属于一个父元素且是紧邻着的
（8）属性选择器（a[rel="external"]）
（9）伪类选择器（a:hover,li:nth-child）给伪类所依附的元素指定样式 动态伪类(h鼠标悬浮于上、a点击链接时、l超链接未访问、v超链接已访问、)
（10）伪元素选择器（::before、::after、::first-item、::first-line) 给虚构的元素指定样式
（11）通配符选择器（*）
```
...
伪类双::,给其所依附的元素添加一个幽灵的样式类
伪元素:,在某个位置添加指定样式的不存在于文档树的虚构元素   
```
...
```
完善了CSS的结构伪类选择器	（:not(.input)：所有class不是“input”的节点）
增加了边框的相关设置比如圆角边框 border-radious
提供了更大的背景元素控制比如多背景
定义了线性和径向两种渐变
增加了更多的文本效果比如text-shadow向文本添加阴影
增加了移动、旋转、缩放相关的转换和变形操作
增加了过度和动画的操作
引入了多列布局和flex弹性盒布局
扩充了媒体查询的功能
```
...
（3） flex中的伸缩项目默认拉伸为父元素的高度，如果项目未设置高度或设为auto，将占满整个容器的高度
的特性，来实现多列等高。
...
#### 17.经常遇到的浏览器的兼容性有哪些？原因，解决方法是什么，常用 hack 的技巧？（X）
...
```
浏览器对CSS样式的兼容性问题，不同的浏览器有不同的默认样式，导致了不同浏览器间样式渲染的不一致性
...
（4）如果元素position:absolute，则“包含块”由最近的position不为static的祖先元的padding box边界形成素建立
如果没有符合条件的祖先元素，则“包含块”是“初始包含块”。
```

#### 21.CSS 里的 visibility 属性有个 collapse 属性值是干嘛用的？在不同浏览器下有什么区别？
...
#### 22.width:auto 和 width:100%的区别（X）
...
<img src="https://gitee.com/kszycyp/blog/raw/master/img/Snipaste_2021-06-01_09-16-48.png" style="zoom: 67%;" />
...
```
浮动的元素，脱离文档流，不会撑开其父盒子，会造成高度会塌陷，而高度的塌陷使我们页面后面的布局不能正常显示。所以清除浮动是为了清除使用浮动元素产生的负面影响，而不是清除浮动本身。
...
媒介都有自己的特点，CSS就是为文档提供在不同媒介上展示的适配方法即针对不同的媒体类型定义不同的样式。
...
包含了一个媒体类型(screen)和至少一-个使用宽度、高度和颜色等媒体属性来限制样式表范围的表达式(max-width)。CSS3加入的媒体查询使得无需修改内容便可以使样式应用于某些特定的设备范围。
```
...
```
SASS（SASS、LESS(Twitter Bootstrap就是采用LESS做底层语言的)没有本质区别，只因为团队前端都是用的SASS）
CSS的相关问题：
语法不够强大，比如无法嵌套书写导致模块化开发中需要书写很多重复的选择器；
没有变量和合理的样式复用机制，使得逻辑上相关的属性值必须以字面量的形式重复输出，导致难以维护。
```

#### 34.CSS 优化、提高性能的方法有哪些？(X)
...
#### 41.absolute 的 containingblock（包含块）计算方式跟正常流有什么不同？(X)
...
#### 44.全屏滚动的原理是什么？用到了 CSS 的哪些属性？（待深入实践）(X)
...
viewport视口，移动端设备屏幕用于显示网页的那一部分区域，viewport可能大于设备上浏览器的可视区域也可能小于。
视口一般分为三种：布局视口layout viewport、视觉视口visual viewport、理想视口ideal viewport、
布局视口大于设备浏览器的可视区域，视觉视口是网页在设备屏幕上所能看到的那一部分内容，理想视口大小等于设备浏览器的可视区域
视口可以通关于viewport属性的meta元素进行相应设置，移动端默认采用的是比可视区域大的布局视口，为了更好的在移动客户端进行使用需要利用meta元素将其转换为理想视口
```
...
#### 46.视差滚动效果，如何给每页做不同的动画？（回到顶部，向下滑动要再次出现，和只出现一次分别怎么做？）(X)
...
#### 47.如何修改 chrome 记住密码后自动填充表单的黄色背景？(X)
...
以前1个css像素=1个设备像素
从 iPhone4 开始，分辨率提高了一倍（640*960），而屏幕尺寸却没变。这时一个css像素=2个设备像素
放大一倍：一个css像素=4个设备像素  元素会占据更多的设备像素
放大1倍，原来 1px 的东西变成 2px，但 1px 变为 2px 并不是把原来的 320px 变为 640px，而是在实际宽度不变的情况下，1px 变得跟原来的 2px 的长度（长宽）一样了（元素会占据更多的设备像素），所以放大1倍后原来需要 320px 才能填满的宽度现在只需要 160px，也即原来 320px 的面积里现在只能填入 160px 的东西了。
...
#### 56.overflow:scroll 时不能平滑滚动的问题怎么处理？(X)
...
```
CSS预处理器是一种专门用于生成CSS文件的新的编程语言，相当于是给css增加了一些编程特性。通俗的说，CSS预处理器用一种专门的编程语言，进行Web页面样式设计，然后再编译成正常的CSS文件。
...
#### 96.隐藏元素的 background-image 到底加不加载？（X）
