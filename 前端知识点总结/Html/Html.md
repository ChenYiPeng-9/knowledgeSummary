...
#### 17. 浏览器的渲染原理？
   ```
    ...
    两者相结合获取render渲染树
    回流(重排、重构)
    重绘(渲染！！)这才是把浏览器请求回来的东西渲染到浏览器窗口的步骤
    渲染的前提是获取渲染树，所以Html、CSS的解析肯定会阻塞渲染。
    全部是在浏览器的渲染引擎上进行的
    ...
    （5）布局阶段结束后是绘制阶段，遍历渲染树并调用渲染对象的 paint 方法将它们的内容显示在屏幕上，绘制使用 UI 基础组
     ...
     html 都解析完成之后再去构建和布局 render 树。它是解析完一部分内容构建一部分内容的dom树然后就显示一部分内容，同时，
     可能还在通过网络下载其余内容。
   ```
   详细资料可以参考：
...
#### 18. 渲染过程中遇到 JS 文件怎么处理？（浏览器解析过程）
   ```
    注：渲染的前提是获取渲染树，所以Html、CSS的解析肯定会阻塞渲染。
    JavaScript 的加载、解析与执行则会阻塞文档的解析，也就是说，在构建 DOM 时，HTML 解析器若遇到了 JavaScript，那么
...
<img src="https://gitee.com/chenyipeng9/blog/raw/master/img/Snipaste_2021-05-24_20-21-43.png" style="zoom:80%;" />

   ```
    理论上，既然样式表不改变 DOM 树，也就没有必要停下文档的解析等待它们，然而，存在一个问题，JavaScript 脚本执行时可
    能在文档的解析过程中请求样式信息，如果样式还没有加载和解析，脚本将得到错误的值，显然这将会导致很多问题。

    所以如果浏览器尚未完成 CSSOM 的下载和构建，而是说，在这种情况下，浏览器会先下载和构建 CSSOM，然后再执行 JavaScript，
    最后再继续文档的解析。
    
    HTML加载解析过程碰到JS脚本，停止解析等JS脚本下载解析执行完成之后在解析
    HTML加载解析过程碰到CSS文件，两者互不干预各自加载各自的并进行解析
    但是如果HTML紧接着又碰到了JS脚本，则HTML停止解析，这个时候也不去下载JS并解析，而是等CSS加载并解析完了，再去加载解析JS，最后恢复HTML的解析，但一般	     只考虑HTML解析过程遇到JS脚本的问题，因为CSS文件的加载解析以及渲染一般是放在HTML文档的head标内也就是说在开始渲染body之前，就已经把相对完整的		CSSOM构建好了。
   ```
...
<img src="https://gitee.com/chenyipeng9/blog/raw/master/img/Snipaste_2021-05-18_22-38-42.png" style="zoom:50%;" />

#### 23. 如何优化关键渲染路径？（浏览器渲染过程）

**关键渲染路径（Critical Rendering Path）是指浏览器将HTML，CSS，JavaScript转换为屏幕上**

**所呈现的实际像素这期间所经历的一系列步骤。**

**关键渲染路径共分五个步骤。构建DOM -> 构建CSSOM -> 构建渲染树 -> 布局 -> 绘制。**

优秀的网站都能够把“首次有效渲染”做到1秒之内完成，这样能够让用户更快的看到所请求的页面得到响应,超过1秒，那么就非常有必要重新分析一下网站的关键渲染路径是否合理

   ```
    为尽快完成首次渲染，在关键渲染路径中，我们通常要关注三个点：：
...
#### 25. 如何减少回流？（浏览器绘制过程）
   ```
     1、浏览器自己的优化：浏览器会维护1个队列，把所有会引起回流、重绘的操作放入这个队列，等队列中的操作到了一定的数量或者到		了一定的时间间隔，浏览器就会flush队列，进行一个批处理。这样就会让多次的回流、重绘变成一次回流重绘。
     
     2、我们要注意的优化：我们要减少重绘和重排就是要减少对渲染树的操作，则我们可以合并多次的DOM和样式的修改。并减少对style		样式的请求。
...
1.web浏览器开始加载解析Html文档，创建document对象，readtstatechange事件状态为loading

2.加载执行同步JS脚本，

3.执行属性为ansync的异步JS(加载过程，与文档解析过程同步进行)

4.Html文档加载解析完成，readystatechange事件变为intercative

5.执行属性为defer的异步JS(加载过程，与文档解析过程同步进行)

6.触发DomcontentLoaded事件

7.此时只是文档加载解析完成了，一些图片啥的外部资源还没有加载完成，当文档中所包含的全部资源都加载完成了，且所有异步JS也都加载执行过了，readystatechange事件变为complete状态，浏览器触发load事件
...
#### 31. b 与 strong 的区别和 i 与 em 的区别？
   ```
    ...
    而 <em> 和 <strong> 是语义样式标签。 <em> 表示一般的强调文本，而 <strong> 表示比 <em> 语义更强的强调文本。吧
...
SEO是一种网站优化技术，也被叫做搜索引擎优化可以利用搜索规则提高网站上有关搜索的自然排名，主要表现为微博热搜控榜等

爬虫不等于搜索引擎, 是搜索引擎一部分, 用于收集信息(网页内容)，因为用来扒网站所以叫爬虫,也叫蜘蛛.
信息收集完后进行离线分析分类索引,才能给真正的搜索引擎部分使用
...
#### 33. HTML5 的离线储存怎么使用，工作原理能不能解释一下？
   ```
  离线存储  离线缓存  应用程序存储   应用程序缓存
  
  在用户没有与因特网连接时，可以正常访问缓存到本地服务器的Web站点或应用，在用户与因特网连接时，更新用户机器上的缓存文件。
    ...
    （1）创建一个拓展名为.appcache 的manifest，然后在页面头部像下面一样加入一个 manifest 的属性。

        <html lang="en" manifest="manifest.appcache">

    （2）在如下cache.manifest 文件的编写离线存储的资源。
      	CACHE MANIFEST
      	#v0.11
      	CACHE:
      	js/app.js
...
redux vuex是单页面应用里的跨页面状态管理（一个Html文档）

这里讲的是多页应用的跨页面数据传递 （多个Html文档）

   ```
    多页应用中多个标签页之间的通信，主要有利用浏览器数据存储方式和服务器方式两大类。本质上都是通过中介者模式来实现的。因为标	签页之间没有办法直接通信，因此我们可以找一个中介者，让标签页和中介者进行通信，然后让这个中介者来进行消息的转发。
    
    方案一：localStorage 对象是充当中介者的角色。在一个标签页里面使用 localStorage.setItem(name,val)、		   		localStorage.removeItem(name)进行数据变动，在另一个标签页里面监听 storage 事件，并从事件对象event的属性
    中获取相关变动的信息
    
    方案二：cookie充当中介者的角色，在一个标签页里将需要传递的消息存储在cookie当中，在另一个标签页里面用setInterval()以	  一定的时间间隔去读取cookie的值。这种方式要求通信的页签必须是同源的，即放在同一个服务器上，然后通过域名的方式去访问这两个	 页签，要不然会认为这两个标签页之间是跨域的，资源是不会被共享的。
    
    方案三：WebSocket是HTML5新增的全双工通信协议，建立起websocket后，服务器可以主动发数据给浏览器，浏览器也可以主动发数	 据给服务器且没有同源限制，客户端可以与任意服务器通信。因此基于WebSocket协议，可以让服务器就充当这个中介者，即通信的标签	 页连接同一个服务器，一个标签页向服务器发送数据后，然后由服务器向其他标签页推送转发。

	方案四：Js是单线程 Html5新增的Webworker实现了客户端的多线程，但是webworker是单页面实现多线程，而shareworker可以实	   现多页面间的多线程。因此我们可以利用ShareWorker在页面存在的生命周期内创建一个唯一的线程，并且开启多个页面也只会使
    用同一个线程，这个时候共享线程就可以充当中介者的角色。标签页间通过共享一个线程，然后通过这个共享的线程来实现数据的交
    换。此外还要注意一点：共享线程的标签页应该是同源的。

   ```
将html文档放到服务器上，通过url地址访问，和直接通过电脑资源路径访问是两回事  

对比http协议，与websocket协议：

首先我们已经有了 HTTP 协议，为什么还需要另一个协议？对于Http协议而言，是建立在TCP协议之上的，TCP协议本身就实现了全双工通信，但是HTTP协议的请求－应答机制限制了全双工通信，也就是说客户端浏览器与服务器端的通信只能由客户端浏览器发起，这种单向请求就存在一个问题，如果服务器端的状态发生连续的变化，我客户端很难去知晓。因此，我们只能使用["轮询"](https://www.pubnub.com/blog/2014-12-01-http-long-polling/)：每隔一段时候，就发出一个询问，了解服务器有没有新的信息。最典型的场景就是聊天室。轮询的效率低，非常浪费资源（因为必须不停连接，或者 HTTP 连接始终打开）。为此，HtML5提出了Websocet协议，该协议是一个全双工通信协议，即服务器可以主动向客户端推送信息，客户端也可以主动向服务器发送信息。
...
#### 41. webSocket 如何兼容低版本浏览器？
   ```
    Adobe Flash Socket 、
    ActiveX HTMLFile (IE) 、
    基于 multipart 编码发送 XHR 、只用一个HTTP请求就可以从服务端向客户端传送多个资源、FireFox
    基于长轮询的 XHR
   ```
...
canvas元素只是一个画布，要调用其下面的一个方法获取绘图对象，绘图对象上存放了基于JS的而相关API
   ...
   ```
    ...
    附加 JavaScript 事件监听函数。并且 SVG 保存的是图形的绘制方法，因此当 SVG 图形缩放时并不会失真。创建XML元素进行绘图。
   ```
   详细资料可以参考：
...
#### 56. 常用的 meta 标签
   ```
	meta是html文档在head标签里定义的一个对文档进行描述的功能性标签，不包含任何内容，
	可提供有关页面的元数据信息（meta-information）。
	meta标签的组成：meta标签共有两个属性，它们分别是http-equiv属性和name属性，不同的属性又有不同的参数值，
	这些不同的参数值就实现了不同的网页功能。
	<meta name="参数"content="具体的参数值"> 主要用于描述网页
	<meta http-equiv="参数"content="参数变量值"> 相当于http的文件头作用
...
解决早期浏览器之间CSS样式不兼容的问题：早期方案css reset，目前方案normalize.css
...
#### 66. 前端性能优化？
   ```
    前端性能优化主要是为了提高页面的响应速度，优化用户的访问体验。此外，web前端响应的时间大部分使用在了资源的加载方面，剩下	 的一小部分用在了资源的解析渲染，因此前端性能优化的绝大部分都是针对资源加载阶段而言的，下面主要从三个方面说一下我的理解：
    
    第一个方面是页面的内容方面：
    注：Web 前端 80% 的响应时间花在图片、样式、脚本等资源下载上。
    (1) 减少 HTTP 请求数，不管是请求资源的逻辑合并(将对不同资源的请求放在一个url地址上)还是利用webpack这样的打包工具实现		 资源的物理合并；使用字体图标、css雪碧图；使用base64编码的行内图片(将图片直接嵌入到html文档中，减少了对资源的http		  请求，增大了文档的大小)；单页面应用程序SPA的路由懒加载。
    （2）通过 DNS 缓存等机制来减少 DNS 的查询次数。
    （3）通过设置缓存策略，对常用不变的资源进行缓存，使用缓存资源进行页面内容展示从而避免的HTTP往返消耗。
    （4）使用延迟加载的方式，来减少页面首屏加载时需要请求的资源(即把首屏用不到的资源进行延迟加载)。延迟加载的资源等首屏加载		 完成或者用户需要访问时，再去请求加载，比如通过懒加载的方式处理非首屏的图片资源。

    
    第二个方面是服务器方面
    注：CDN放静态资源的服务器（如JS，CSS, 图片, 字体...）可以用来加速静态资源的下载。
    （1）内容分发网络（CDN）是一组分散在不同地理位置的web服务器，选择离用户最近的服务器来发送内容的，这可以提高用户对于资源		   请求时的响应速度。此外， CDN 服务器的地址一般都跟主服务器的地址不同，所以可以破除浏览器对同一个域名发送请求的限制。
    	补充：对于同一个协议、域名、端口，浏览器允许同时打开最多 6个 TCP 连接（比如访问淘宝最多同时只能发送 6个请求），		Http2.0引入了多路复用的机制，可以最大化发送请求数量。
	（2）服务器端启用 Gzip对传输的资源进行压缩，减小请求资源文件的体积，包括JS、CSS、图片等资源。
		注：Accept-Encoding HTTP请求头告知web服务器用客户端列出的一种方式来压缩响应，web服务器通过Content-Encoding		  响应头来通知客户端
	（3）尽可能减小 cookie 的大小，并且通过将静态资源分配到其他域名下，来避免对静态资源请求时携带不必要的 cookie
	
    第三个方面是 CSS、JavaScript、图片等请求资源方面 
    
    （1）把样式表放在页面的 head 标签中，减少页面的首次渲染的时间；不要使用CSS表达式；使用<link>替代@import
    （2）尽量把 js 脚本放在页面底部或者使用 defer 或 async 属性，避免脚本的加载和执行阻塞页面的渲染。
    （3）导致浏览器卡顿的原因一般都是操作 DOM的次数太频繁，因此尽可能的减少 DOM 的操作，比如Vue的虚拟 DOM，本质上就是用 			JS数据来模拟真实 DOM树，从而大大减少了操作真实DOM 的次数。
    （4）可以使用加载速度更快的WebP 格式的图片。
    
    浏览器缓存主要是指Http协议缓存，可以通过meta元素、以及http头进行相关缓存的设置，但一般是用 HTTP 头信息来控制缓存。浏		览器缓存分为强缓存和协商缓存。
    强缓存不发请求到服务器 直接拿缓存，协商缓存会发请求到服务器 服务器告诉你 去拿缓存 就拿 不拿缓存就拿新的代码。大部分web		服务器都默认开启协商缓存
    
    强缓存  Expires Cache-Control 设置缓存的有效时间来判断是否命中强缓存，即对强缓存进行控制，前者是绝对时间，后者是相对	  时间（由多个字段构成，主要是max-age缓存有效的相对时间），后者优先级高。
    
    协商缓存 资源最后修改时间是否改变(Last-Modify/If-Modify-Since),资源校验码是否改变(ETag/If-None-Match)来判断是	否命中协商缓存。
